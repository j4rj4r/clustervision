...
async def list_cluster_roles(
    ...
    return await loop.run_in_executor(None, svc.list_cluster_roles, include_system)
...
async def delete_cluster_role(name: str, svc: RbacService = Depends(get_rbac_service)):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, svc.delete_cluster_role, name)
...
async def list_roles(namespace: str, svc: RbacService = Depends(get_rbac_service)):
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, svc.list_roles, namespace)
...
async def delete_role(namespace: str, name: str, svc: RbacService = Depends(get_rbac_service)):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, svc.delete_role, namespace, name)
...
async def list_cluster_bindings(svc: RbacService = Depends(get_rbac_service)):
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, svc.list_cluster_role_bindings)
...
async def create_cluster_binding(payload: BindingCreate, svc: RbacService = Depends(get_rbac_service)):
    ...
    return await loop.run_in_executor(
        None, svc.create_cluster_role_binding, payload.name, payload.role_name, payload.subjects
    )
...
async def delete_cluster_binding(name: str, svc: RbacService = Depends(get_rbac_service)):
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, svc.delete_cluster_role_binding, name)
...
async def list_namespace_bindings(namespace: str, svc: RbacService = Depends(get_rbac_service)):
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, svc.list_role_bindings, namespace)
...
async def delete_namespace_binding(
    ...
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, svc.delete_role_binding, namespace, name)
...
async def get_user_permissions(username: str, svc: RbacService = Depends(get_rbac_service)):
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, svc.get_user_permissions, username)
...
async def revoke_role(
    ...
    loop = asyncio.get_event_loop()
    await loop.run_in_executor(None, svc.revoke_role, username, role_name, namespace)
...
async def list_namespaces(svc: RbacService = Depends(get_rbac_service)):
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(None, svc.list_namespaces)
